...
def downsample_transform(images, size = (16, 16)):
	'''
	Downsample images to size. 
	'''
	downsized_images = list(map(lambda x: resize(x, size, anti_aliasing= True), images))
	downsized_images = np.stack(downsized_images)
	return downsized_images

def process_data(data_chunk, depth = 10, downsample = False, flatten = False):
	"""
	Standardise the images and one-hot encode the labels.
	"""
	images, label = tf.cast(data_chunk['image'], tf.float64), data_chunk['label']
	...
	# Standardise
	standardised_images = standardise_images(images)
	if flatten == True:
		standardised_images = np.reshape(standardised_images, (images.shape[0], -1))
	label = tf.one_hot(label, depth = depth)
	label = label - 0.1
	return {'image': onp.array(standardised_images, dtype = onp.float64), 'label': label.numpy()}

def generate_datasets(train_size, test_size = 20, type = 'cifar10', downsample = False, flatten = False):
	'''
	...
	if type == 'cifar10':
		...
		ds_train, ds_test = tfds.load(
			'mnist', split=['train[:%d]' % train_size,'test[:%d]' % test_size], batch_size=-1)
	elif type == 'fashion_mnist':
		ds_train, ds_test = tfds.load(
			'fashion_mnist', split=['train[:%d]' % train_size,'test[:%d]' % test_size], batch_size=-1)
		
	permutation = onp.random.RandomState(seed=42).permutation(train_size)
	train_data = process_data(ds_train, downsample = downsample, flatten = flatten) 
	train_data['image'] = train_data['image'][permutation]
	train_data['label'] = train_data['label'][permutation]
	test_data = process_data(ds_test, downsample = downsample, flatten = flatten)
...
# def generate_datasets(train_size, test_size = 20, type = 'cifar10', flatten = False):
# 	'''
# 	Load raw dataset and then preprocess them. 
# 	'''
# 	if type == 'cifar10':
